...
import { p as prismExports } from './prism-python-uEphcfi3.js';
import { s as sanitize_html_ } from './index50-nVz3eHRG.js';
...
const standardHtmlAndSvgTags = [
  ...standardHtmlTags,
  ...svgTags.filter((tag2) => !standardHtmlTags.includes(tag2))
];
  ...
  $$renderer.component(($$renderer2) => {
    ...
    function escapeTags(content, tagsToEscape) {
      ...
      if (Array.isArray(tagsToEscape)) {
        const tagPattern = tagsToEscape.map((tag2) => ({
          open: new RegExp(`<(${tag2})(\\s+[^>]*)?>`, "gi"),
          close: new RegExp(`</(${tag2})>`, "gi")
        }));
        let result = content;
    ...
    $$renderer2.push(`<span${attr_class("md svelte-1hf8a14", void 0, { "chatbot": chatbot, "prose": render_markdown })}>${html$1(html2)}</span>`);
  });
}

export { MarkdownCode as M };
//# sourceMappingURL=MarkdownCode-BPzQM9w-.js.map
